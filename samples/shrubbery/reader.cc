...
namespace shrubbery
{
  ...
  PassDef check_parsing()
  {
    return {
      ...
      dir::bottomup | dir::once,
      {
        ...
        (T(Group) << (!T(Block))++[Terms] * (T(Block) << End)) *
            (T(Group) << T(Alt))[Alt] >>
          [](Match& _) { return Seq << (Group << _[Terms]) << _(Alt); },

        (T(Block) << (T(Group)[Group] << T(Alt))) >>
          [](Match& _) { return Seq << *_[Group]; },

        // An empty group caused by a semicolon is ignored
        (In(Semi) * ((T(Group) << End))) >> [](Match&) { return Seq ^ ""; },

        // Commas must separate (non-empty) groups
        (T(Comma) << End)[Comma] >>
          [](Match& _) {
            return err(_[Comma], "Comma does not separate groups");
          },

        (In(Comma) * (T(Group) << End)[Group]) >>
          [](Match& _) {
            return err(_[Group], "Comma does not separate groups");
          },
        ...
        ((--In(Paren, Brace, Bracket)) * T(Comma)[Comma]) >>
          [](Match& _) {
            return err(
              _[Comma],
              "Commas can only separate groups in parentheses/braces/brackets");
          },
        ...
        (In(Paren, Brace, Bracket) * Any * Any)[Group] >>
          [](Match& _) {
            return err(
              _[Group],
              "Groups in parentheses/braces/brackets must be comma separated");
          },
        ...
        (In(Paren, Brace, Bracket, Comma) * T(Semi))[Semi] >>
          [](Match& _) {
            return err(
              _[Semi],
              "Semicolons cannot separate groups in "
              "parentheses/brackets/braces. Use commas.");
          },

        // Blocks cannot be empty, except immediately under opener-closer pairs
        // and as the only term in a top-level group
        (--(In(Paren, Brace, Bracket, Comma, File))) *
            ((T(Group) << ((!T(Block))++ * (T(Block)[Block] << End)))) >>
          [](Match& _) { return err(_[Block], "Blocks may not be empty"); },

        In(File) *
            (T(Group) << ((
               (!T(Block)) * (!T(Block))++ * (T(Block)[Block] << End) *
               End))) >>
          [](Match& _) { return err(_[Block], "Blocks may not be empty"); },
  ...
  PassDef merge_alternatives()
  {
    return {
      ...
      dir::bottomup | dir::once,
      {
        ...
        (--In(Comma, Semi) * T(Group)[Group] *
         ((T(Group) << T(Alt)[Alt]) * (T(Group) << T(Alt))++[Terms])) >>
          [](Match& _) {
            Node alt = _(Alt);
  ...
  PassDef drop_separators()
  {
    return {
      ...
      dir::bottomup | dir::once,
      {
        (T(Comma)[Comma]) >> [](Match& _) { return Seq << *_[Comma]; },

        T(Semi)[Semi] >> [](Match& _) { return Seq << *_[Semi]; },
      }};
  }
    ...
    return {
      ...
      dir::bottomup | dir::once,
      {
        (--In(Brace, Bracket)) * T(Group) << T(Alt)[Alt] >>
          [](Match& _) {
            return err(_[Alt], "Alternative cannot appear first in a group");
          },
      }};
  }
  ...
  PassDef group_structure()
  {
    return {
      ...
      dir::bottomup | dir::once,
      {
        In(Group) * Start * (!T(Block, Alt))++[Atom] * ~T(Block)[Block] *
            ~T(Alt)[Alt] * End >>
          [](Match& _) {
            return Seq << (Terms << _[Atom]) << (_(Block) ? _(Block) : None)
                       << (_(Alt) ? _(Alt) : None);
          },
