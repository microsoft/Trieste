...
namespace shrubbery
{
  ...
  Parse parser()
  {
    ...
    // Push a new indentation level. Will only have an effect if we are
    // expecting a new indentation
    auto push_indentation =
      [check_new_indentation, expect_indent, indent](auto& m) {
        if (!*expect_indent)
          return;
    ...
    // Pop the latest established indentation level, unless we are currently
    // expecting a new indentation
    ...
    // Figure out which indentation level we are currently at (and ensure that
    // it is valid)
    auto match_indentation = [pop_indentation, newline, indent](auto& m) {
      ...
      *newline = false;
      ...
      while (col < last_col)
      {
        ...
        {
          m.error(
            "Indentation is before the first group of a comma-separated list");
          return false;
        }
    ...
    // Continue the current indentation level. Will only have an effect if we
    // are expecting a new indentation
    auto continue_indentation =
      [push_indentation, expect_indent, indent](auto& m) {
        if (*expect_indent)
        {
          return false;
        }
    ...
    auto close_all = [pop_indentation, expect_indent, indent](
                       auto& m, std::initializer_list<Token> tokens) {
      while (m.in(tokens) || m.group_in(tokens))
      {
        ...
        if (
          !indent->empty() && !*expect_indent &&
          (token == Block || token == Alt))
        {
          pop_indentation();
        }
    ...
    auto open_pair =
      [match_indentation, push_indentation, expect_indent](auto& m) {
        push_indentation(m);
    ...
    // Adds a term, defaulting to Atom (anything that is not a special
    // character)
    auto add_term = [match_indentation, push_indentation, newline](
                      auto& m, Token token = Atom) {
      if (*newline)
        m.term();
    ...
    p("start",
      {// Whitespace between tokens.
       ...
       // String literals
       R"("[^"]*")" >> [add_term](auto& m) { add_term(m); },

       // Identifiers
       R"([[:alpha:]_][[:alnum:]_]*)" >> [add_term](auto& m) { add_term(m); },

       // Integers
       "[[:digit:]]+" >> [add_term](auto& m) { add_term(m); },
