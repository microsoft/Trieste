...
namespace trieste
{
  namespace yaml
  {
    Parse parser()
    {
      ...
      // YAML starts out with the possibility of one or more directives that
      // will apply to a subsequent document.
      p("directives",
        {
          R"([ \t]+)" >> [](auto&) { return; },
          ...
          // YAML directive
          // %YAML[ \t]+ : The text "%YAML" followed by one or more spaces or
          // tabs [0-9] : A single digit
      ...
      p("document",
        {
          ...
          // characters
          // (?:[ \t]+[^\s]+) : One or more spaces or tabs followed by one or
          // more characters which are NOT whitespace
          // ([ \t]+#[^\r\n]*)? : Zero or one spaces or tabs followed by a
          // comment (optional)
          R"((%[[:alpha:]]+(?:[ \t]+[^\s]+))([ \t]+#[^\r\n]*)?)" >>
            [](auto& m) {
          ...
          // (?:[ \t]+|\r?(\n)) : Either one or more spaces or tabs, or a
          // newline
          R"(([[a-zA-Z0-9\?:-](?:[^\s]|[^:\r\n] [^\s#])*) *(:)(?:[ \t]+|\r?(\n)))" >>
            [anchors](auto& m) {
              m.add(Value, 1);
          ...
          R"((\*([^\[\]\{\}\, \r\n]+)(:))(?:[ \t]+|\r?(\n)))" >>
            [anchors](auto& m) {
              ...
              {
                // this is not a map key, but rather an alias that ends in a
                // colon
                m.add(Alias, 2);
                m.extend(Alias, 3);
              }
          ...
          // ns-shorthand-tag
          // ![0-9A-Za-z\-]*!|!!|! : An exclamation mark followed by zero or
          // more alphanumeric characters or hyphens,
          //                         followed by an exclamation mark, or two
          //                         exclamation marks, or a single exclamation
          //                         mark
          // (?:[\w#;\/\?:@&=+$_.~*'()]|%\d\d)+ : One or more of: a word
          // character and #;/?:@&=+$_.~*'(),
          ...
          // [^\s:\?-] : A character which is NOT whitespace, a colon, a
          // question mark, or a hyphen
      ...
      // Flow mode is very similar to document mode, but provides extra
      // characters to disambiguate the structure of mappings and sequences.
      // These characters were chosen such that flow mode acts a superset of
      // JSON. However, importantly, it is NOT JSON, and document-style
      // constructs can still appear within flow mode.
      p("flow",
        {
          "---" >>
      ...
      p.gen({
        ...
        VerbatimTag >>
          [](auto& rnd) { return "<" + rand_string(rnd, 1) + ">"; },
